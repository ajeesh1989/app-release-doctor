...
const WORKSPACE_ROOT = path.join(
  os.tmpdir(),
  "app-release-doctor-remote",
);

const AAB_ROOT = path.join(
  WORKSPACE_ROOT,
  "aab",
);
...
function sanitizeAabFileName(fileName: string): string {
...
export async function ensureRemoteWorkspaceRoot(): Promise<void> {
...
export async function createAabWorkspace(
  originalFileName = "app.aab",
): Promise<{
...
export async function createProjectWorkspace(): Promise<{
  workspace: RemoteWorkspace;
  projectPath: string;
}> {
  await ensureRemoteWorkspaceRoot();
...
export async function resolveAabWorkspace(
  uploadId: string,
): Promise<{
  workspace: RemoteWorkspace;
  aabPath: string;
  ...
  try {
    directoryStats = await fs.stat(
      directory,
    );
  } catch {
    throw new Error(
      "AAB upload was not found. It may have expired or been removed.",
    );
  }

  if (!directoryStats.isDirectory()) {
    throw new Error(
      "Invalid AAB workspace.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "AAB upload has expired. Please upload the AAB again.",
    );
  }
  ...
  if (aabFiles.length === 0) {
    throw new Error(
      "Uploaded AAB file was not found.",
    );
  }

  if (aabFiles.length > 1) {
    throw new Error(
      "Invalid AAB workspace. Multiple AAB files were found.",
    );
  }
...
export async function resolveProjectWorkspace(
  ...
  if (
    ...
    throw new Error(
      "Invalid Flutter project upload ID.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "Flutter project upload was not found. It may have expired or been removed.",
    );
  }

  if (!directoryStats.isDirectory()) {
    throw new Error(
      "Invalid Flutter project workspace.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Flutter project upload has expired. Please upload the project again.",
    );
  }
  ...
  try {
    const pubspecStats = await fs.stat(
      pubspecPath,
    );

    if (!pubspecStats.isFile()) {
      throw new Error(
        "Flutter project does not contain a valid pubspec.yaml.",
      );
    }
  } catch (error) {
    if (
    ...
    throw new Error(
      "Flutter project does not contain pubspec.yaml.",
    );
  }
...
export async function cleanupExpiredRemoteWorkspaces(): Promise<void> {
  await ensureRemoteWorkspaceRoot();
...
export async function cleanupRemoteWorkspaces(): Promise<void> {
  await fs.rm(
    WORKSPACE_ROOT,
    {
      recursive: true,
...
export function getRemoteWorkspaceRoot(): string {
