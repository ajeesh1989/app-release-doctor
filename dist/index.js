...
import { z } from "zod";
import { resolveAabWorkspace, resolveProjectWorkspace, } from "./remote-workspace.js";
...
function extractString(text, patterns) {
    for (const pattern of patterns) {
        const match = text.match(pattern);
        if (match?.[1]) {
            return match[1].trim();
...
function detectReleaseSigning(gradleText) {
    ...
    const hasReleaseSigningReference = /signingconfig\s*=\s*signingconfigs\.getbyname\s*\(\s*["']release["']\s*\)/i.test(gradleText) ||
        /signingconfig\s+signingconfigs?\.release/i.test(gradleText) ||
        /signingconfig\s*=\s*signingconfigs?\.release/i.test(gradleText) ||
        /signingconfig\s+signingconfigs?\.getbyname\s*\(\s*["']release["']\s*\)/i.test(gradleText);
    // Keystore properties are another strong indicator
    const hasKeystoreProperties = /key\.properties/i.test(gradleText) &&
        /storefile/i.test(gradleText) &&
        /storepassword/i.test(gradleText) &&
        /keyalias/i.test(gradleText) &&
        /keypassword/i.test(gradleText);
    return (hasReleaseSigningConfig &&
        (hasReleaseSigningReference ||
            hasKeystoreProperties));
}
function extractSdkValue(gradleText, property) {
    ...
    return extractNumber(gradleText, patterns);
}
...
function registerTools(server) {
    server.tool("check_target_sdk", "Check whether an Android target SDK meets the expected Play release requirement.", {
        targetSdk: z
            .number()
            .int()
            .positive(),
...
flutter build appbundle --release
    ...
    // ============================================================
    server.tool("check_flutter_project", "Inspect a local or remotely uploaded Flutter project for Android release configuration.", {
        projectPath: z.string().optional(),
        uploadId: z.string().optional(),
    }, async ({ projectPath, uploadId, }) => {
...
For a remotely uploaded Flutter project, provide the uploadId returned by the project upload service.
          `.trim());
            ...
            // --------------------------------------------------------
            if (uploadId) {
                const resolved = await resolveProjectWorkspace(uploadId);
...
${friendlyPath(normalizedProjectPath)}`}

Please upload the project again or provide a valid Flutter project root.
          `.trim());
            }
            const projectStats = fs.statSync(normalizedProjectPath);
...
Make sure you provide the Flutter project root or upload a valid Flutter project ZIP.
          `.trim());
            ...
            // READ GRADLE
            // --------------------------------------------------------
            const gradlePath = getBuildGradlePath(normalizedProjectPath);
            ...
            // --------------------------------------------------------
            const compileSdk = extractSdkValue(gradleText, "compileSdk");
            const targetSdk = extractSdkValue(gradleText, "targetSdk");
            const minSdk = extractSdkValue(gradleText, "minSdk");
            const namespace = extractString(gradleText, [
                /namespace\s*=\s*["']([^"']+)["']/i,
                /namespace\s+["']([^"']+)["']/i,
            ]);
            const applicationId = extractString(gradleText, [
                /applicationId\s*=\s*["']([^"']+)["']/i,
                /applicationId\s+["']([^"']+)["']/i,
            ]);
            const releaseSigning = detectReleaseSigning(gradleText);
            const flutterVersion = extractString(pubspecText, [
                /^version:\s*([^\r\n]+)/m,
            ]);
            // --------------------------------------------------------
            // OUTPUT
            ...
            output.push("FLUTTER PROJECT CHECK");
            output.push("");
            output.push("PROJECT");
            output.push("-------");
            if (remoteUpload) {
                output.push("Source: Remote uploaded Flutter project");
            }
            else {
                output.push(`Path: ${friendlyPath(normalizedProjectPath)}`);
            }
            output.push("🟢 Flutter project detected");
            output.push("");
            output.push("ANDROID CONFIGURATION");
            output.push("---------------------");
            output.push(`Compile SDK: ${compileSdk ?? "Not detected"}`);
            output.push(`Target SDK: ${targetSdk ?? "Not detected"}`);
            output.push(`Min SDK: ${minSdk ?? "Not detected"}`);
            output.push(`Application ID: ${applicationId ??
                "Not detected"}`);
            output.push(`Namespace: ${namespace ??
                "Not detected"}`);
            output.push("");
            output.push("RELEASE SIGNING");
            output.push("---------------");
            output.push(releaseSigning
                ? "🟢 Release signing configuration detected."
                : "🟡 Release signing configuration was not clearly detected.");
            output.push("");
            output.push("FLUTTER VERSION");
            output.push("---------------");
            output.push(flutterVersion ??
                "Version not detected.");
            output.push("");
            output.push("TARGET SDK STATUS");
            output.push("-----------------");
            if (targetSdk === null) {
                output.push("🟡 Target SDK could not be detected automatically.");
            }
            else if (targetSdk >= 36) {
                output.push(`🟢 Target SDK ${targetSdk} is good.`);
            }
            else {
                output.push(`🔴 Target SDK ${targetSdk} is below the expected target SDK 36.`);
            }
            output.push("");
            output.push("NEXT STEP");
            output.push("---------");
            output.push("👉 Run the Play Store readiness check.");
            output.push("👉 Build a release AAB.");
            output.push("👉 Run Smart AAB Inspection.");
...
Please verify the project and try again.
    ...
    // ============================================================
    server.tool("check_play_store_readiness", "Check a local or remotely uploaded Flutter project for common Android release readiness items.", {
        projectPath: z.string().optional(),
        uploadId: z.string().optional(),
    }, async ({ projectPath, uploadId, }) => {
        let normalizedProjectPath = "";
...
Please provide the Flutter project root or upload a valid Flutter project ZIP.
          `.trim());
            ...
            const signingConfigured = detectReleaseSigning(gradleText);
            // --------------------------------------------------------
            ...
            checks.push({
                ...
                passed: gradlePath !== null,
                detail: gradlePath
                    ? `Detected: ${path.basename(gradlePath)}`
                    : "Android app Gradle file was not detected.",
            });
            checks.push({
                name: "Compile SDK",
                passed: compileSdk !== null &&
                    compileSdk >= 36,
                detail: compileSdk === null
                    ? "Could not detect compileSdk."
                    : `compileSdk ${compileSdk}`,
            });
            checks.push({
                name: "Target SDK",
                passed: targetSdk !== null &&
                    targetSdk >= 36,
                detail: targetSdk === null
                    ? "Could not detect targetSdk."
                    : `targetSdk ${targetSdk}`,
            });
            checks.push({
                name: "Application ID",
                passed: Boolean(applicationId),
            ...
            const output = [];
            output.push("🩺 APP RELEASE DOCTOR");
            output.push("PLAY STORE READINESS");
            output.push("");
            output.push("PROJECT");
            output.push("-------");
            if (remoteUpload) {
            ...
            output.push("");
            output.push(`RESULT: ${failedCount === 0
                ? "🟢 READY"
                : "🟡 NEEDS REVIEW"}`);
            output.push("");
            output.push(`Passed: ${passedCount}/${checks.length}`);
            output.push("");
            // --------------------------------------------------------
            // INDIVIDUAL CHECKS
            // --------------------------------------------------------
            for (const check of checks) {
                output.push(`${check.passed
                    ? "🟢"
                    : "🔴"} ${check.name}`);
            ...
            // DETECTED VALUES
            // --------------------------------------------------------
            output.push("DETECTED VALUES");
            output.push("---------------");
            output.push(`Compile SDK: ${compileSdk ??
                "Not detected"}`);
            output.push(`Target SDK: ${targetSdk ??
                "Not detected"}`);
            output.push(`Min SDK: ${minSdk ??
                "Not detected"}`);
            output.push(`Application ID: ${applicationId ??
                "Not detected"}`);
            output.push(`Flutter Version: ${flutterVersion ??
                "Not detected"}`);
            output.push("");
            // --------------------------------------------------------
            // RELEASE SIGNING
            ...
            output.push("---------");
            if (failedCount === 0) {
                output.push("🟢 The project passed the current App Release Doctor readiness checks.");
                output.push("");
                output.push("👉 Build your release AAB.");
                output.push("👉 Run Smart AAB Inspection.");
            }
            else {
                output.push("👉 Review the failed checks above.");
                output.push("👉 Fix the relevant Android release configuration.");
                output.push("👉 Build a fresh release AAB.");
            }
...
Please verify the project and try again.
    ...
    // ============================================================
    server.tool("build_release", "Build a local or remotely uploaded Flutter Android App Bundle in release mode.", {
        projectPath: z.string().optional(),
        uploadId: z.string().optional(),
    }, async ({ projectPath, uploadId, }) => {
        let normalizedProjectPath = "";
...
Please provide the Flutter project root or upload a valid Flutter project ZIP.
          `.trim());
            ...
            // BUILD RELEASE AAB
            // --------------------------------------------------------
            const result = await execAsync("flutter build appbundle --release", {
            ...
            // FIND GENERATED AAB
            // --------------------------------------------------------
            const possibleAab = path.join(normalizedProjectPath, "build", "app", "outputs", "bundle", "release", "app-release.aab");
            if (!fs.existsSync(possibleAab)) {
                return textResult(`
...
Please check the Flutter build output.
          `.trim());
            ...
            // BUILD SUCCESS
            // --------------------------------------------------------
            const stats = fs.statSync(possibleAab);
            return textResult(`
...
${remoteUpload
                ...
                : `Path: ${friendlyPath(normalizedProjectPath)}`}
...
${remoteUpload
    ...
    // TOOL 5 — ANALYZE AAB
    // ============================================================
    server.tool("analyze_aab", "Check whether a local Android App Bundle exists and is readable.", {
        aabPath: z.string(),
    }, async ({ aabPath }) => {
...
For a remote uploaded AAB, provide the uploadId returned by the upload service.
          `.trim());
            ...
            // --------------------------------------------------------
            if (uploadId) {
                const resolved = await resolveAabWorkspace(uploadId);
                normalizedAabPath =
                    resolved.aabPath;
            }
            // --------------------------------------------------------
...
👉 Build a fresh AAB and try again.
          `.trim());
            ...
            // LARGEST FILES
            // --------------------------------------------------------
            const largestFiles = [...meaningfulEntries]
                .sort((a, b) => b.size - a.size)
                .slice(0, 15);
            const largestImages = [...images]
                .sort((a, b) => b.size - a.size)
                .slice(0, 10);
            const largestAudio = [...audio]
                .sort((a, b) => b.size - a.size)
                .slice(0, 10);
            ...
            for (const entry of nativeLibraries) {
                ...
                const match = normalizedName.match(/(?:^|\/)base\/lib\/([^/]+)\//i);
                if (match?.[1]) {
            ...
            if (nativeLibraries.length >
                ...
                const largestNative = [...nativeLibraries]
                    .sort((a, b) => b.size -
                    a.size)
                    .slice(0, 6);
...
// ============================================================
const currentFile = path.resolve(new URL(import.meta.url).pathname);
