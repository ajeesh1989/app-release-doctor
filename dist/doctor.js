...
function getBuildGradlePath(projectPath) {
    ...
    const groovy = path.join(projectPath, "android", "app", "build.gradle");
...
function detectReleaseSigning(content) {
    return (/signingConfigs\s*\{[\s\S]*?(release|create\s*\(\s*["']release["']\s*\))/i.test(content) &&
        /signingConfig/i.test(content));
}
function extractSdkValue(content, type) {
    const pattern = type === "compileSdk"
        ? /compileSdk\s*=?\s*(\d+)/i
        : /targetSdk\s*=?\s*(\d+)/i;
...
function extensionMatches(name, expression) {
    return expression.test(name);
}
// ============================================================
// SMART AAB INSPECTION
// ============================================================
export async function inspectAab(aabPath) {
    let tempScript = null;
    try {
        ...
        if (!fileStats.isFile()) {
            return {
                ...
                score: 0,
                health: "INVALID",
                report: "🔴 THAT PATH IS NOT AN AAB FILE",
            };
        }
        tempScript = path.join(projectTempDirectory(), `smart-aab-${Date.now()}.ps1`);
...
`;
        ...
        try {
            ...
            return {
                ...
                score: 0,
                health: "FAILED",
                report: `🔴 I COULDN'T INSPECT THE AAB\n\n${e.stderr ||
                    e.stdout ||
                    e.message ||
                    String(error)}`,
            };
        ...
        if (lines.length === 0) {
            return {
                ...
                score: 0,
                health: "FAILED",
                report: "🔴 NO AAB CONTENT WAS RETURNED",
            };
        }
        const entries = [];
        for (const line of lines) {
            ...
            const name = line.substring(0, first);
            const sizeText = second === -1
                ? line.substring(first + 1)
                : line.substring(first + 1, second);
            const sha256 = second === -1
                ? ""
        ...
        const images = meaningfulEntries.filter((entry) => extensionMatches(entry.name, /\.(png|jpg|jpeg|webp|gif|bmp|heic|avif)$/i));
        const audio = meaningfulEntries.filter((entry) => extensionMatches(entry.name, /\.(mp3|wav|ogg|m4a|aac|flac)$/i));
        const videos = meaningfulEntries.filter((entry) => extensionMatches(entry.name, /\.(mp4|mov|mkv|webm|avi)$/i));
        const fonts = meaningfulEntries.filter((entry) => extensionMatches(entry.name, /\.(ttf|otf|woff|woff2)$/i));
        ...
        if (!hasBaseManifest) {
            findings.push({
                ...
                reason: "The expected base Android manifest was not found.",
                action: "Verify this is a complete release AAB and rebuild it.",
            });
        }
        if (dexFiles.length === 0 &&
            ...
            findings.push({
                ...
                reason: "No DEX files or native libraries were found.",
                action: "Build a fresh release AAB using flutter build appbundle --release.",
            });
        }
        ...
        if (veryLargeImages.length > 0) {
            findings.push({
                ...
                action: "Consider reducing resolution or compressing these images.",
            });
        }
            ...
            findings.push({
                ...
                action: "Review whether these images can be resized or compressed.",
            });
        }
        const veryLargeAudio = audio.filter((item) => item.size >=
            10 * 1024 * 1024);
        const largeAudio = audio.filter((item) => item.size >=
            5 * 1024 * 1024);
        if (veryLargeAudio.length > 0) {
            findings.push({
                ...
                action: "Consider reducing bitrate or using more efficient encoding.",
            });
        }
            ...
            findings.push({
                ...
                action: "Review bitrate and encoding.",
            });
        }
        const veryLargeVideos = videos.filter((item) => item.size >=
            50 * 1024 * 1024);
        const largeVideos = videos.filter((item) => item.size >=
            20 * 1024 * 1024);
        if (veryLargeVideos.length > 0) {
            findings.push({
                ...
                action: "Consider reducing video resolution, bitrate, or duration.",
            });
        }
            ...
            findings.push({
                ...
                action: "Review video encoding and resolution.",
            });
        }
        if (duplicateGroups.length > 0 &&
            ...
            findings.push({
                ...
                action: "Check whether repeated assets are intentionally included more than once.",
            });
        }
            ...
            findings.push({
                ...
                reason: `${duplicateGroups.length} duplicate group(s) represent approximately ${formatMB(duplicateBytes)} MB of repeated content.`,
                action: "Review duplicates if you want to reduce application size.",
            });
        }
        if (meaningfulContentSize >
            ...
            findings.push({
                ...
                action: "Review the largest assets.",
            });
        }
            ...
            findings.push({
                ...
                reason: `Meaningful application content totals approximately ${formatMB(meaningfulContentSize)} MB uncompressed.`,
                action: "Review large assets if reducing size matters.",
            });
        }
        if (architectures.size >= 3) {
            findings.push({
                ...
                action: "This can be normal for Flutter applications.",
            });
        }
        if (nativeLibraries.length > 0) {
            findings.push({
                ...
                action: "This is normal for Flutter applications and many plugins.",
            });
        }
        if (debugMetadataEntries.length > 0) {
            findings.push({
                ...
                reason: `${debugMetadataEntries.length} debug/build metadata file(s) detected.`,
                action: "These entries are excluded from release health calculations.",
            });
        }
        const problemFindings = findings.filter((f) => f.kind === "PROBLEM");
        const optimizationFindings = findings.filter((f) => f.kind === "OPTIMIZATION");
        const informationFindings = findings.filter((f) => f.kind === "INFO");
        let score = 100;
        let high = 0;
        let medium = 0;
        let low = 0;
        ...
        output.push("---------------------");
        output.push(architectures.size > 0
            ? Array.from(architectures).join(", ")
        ...
        const largestNative = [...nativeLibraries]
            .sort((a, b) => b.size - a.size)
            .slice(0, 6);
        for (const native of largestNative) {
            output.push(`• ${formatMB(native.size)} MB — ${native.name}`);
        }
        output.push("");
        output.push("RELEASE RISKS");
        output.push("-------------");
        output.push(`🔴 High: ${high}`);
        output.push(`🟠 Medium: ${medium}`);
        output.push(`🟡 Low: ${low}`);
        output.push("");
        if (problemFindings.length === 0) {
            ...
            let number = 1;
            for (const finding of problemFindings) {
                output.push(`${number}. ${finding.title}`);
        ...
        if (duplicateGroups.length === 0) {
            ...
            output.push("");
            for (let i = 0; i <
                Math.min(5, duplicateGroups.length); i++) {
        ...
        output.push("---------------");
        if (problemFindings.length > 0) {
            output.push("🔴 Release risks were detected. Review them before release.");
        }
        else if (optimizationFindings.length > 0) {
            output.push("🟢 No obvious release risks were detected. Remaining findings are optional optimizations.");
        }
        else {
        ...
        if (problemFindings.length > 0) {
            ...
            output.push("👉 Build a fresh AAB after fixing them.");
        }
        else {
...
// ============================================================
export async function checkFlutterProject(projectPath) {
    const normalized = friendlyPath(projectPath);
...
`.trim();
    ...
    const gradle = getBuildGradlePath(normalized);
    if (!gradle) {
        return `
...
`.trim();
    ...
    const gradleContent = fs.readFileSync(gradle, "utf8");
    const pubspec = fs.readFileSync(path.join(normalized, "pubspec.yaml"), "utf8");
    const compileSdk = extractSdkValue(gradleContent, "compileSdk");
    const targetSdk = extractSdkValue(gradleContent, "targetSdk");
    const compileInherited = /compileSdk\s*=?\s*flutter\.compileSdkVersion/i.test(gradleContent);
    const targetInherited = /targetSdk\s*=?\s*flutter\.targetSdkVersion/i.test(gradleContent);
    const applicationId = extractString(gradleContent, /applicationId\s*=?\s*"([^"]+)"/i);
    const version = extractString(pubspec, /^version:\s*([^\s#]+)$/m);
    ...
    const compileOk = compileSdk !== null &&
        compileSdk >= 36;
    const targetOk = targetSdk !== null &&
        targetSdk >= 36;
    const signingOk = detectReleaseSigning(gradleContent);
    const appIdOk = !!applicationId;
    const versionOk = !!versionName;
    const versionCodeOk = !!versionCode &&
        /^\d+$/.test(versionCode);
    const output = [];
    output.push(compileOk ||
        compileInherited
    ...
    output.push("");
    output.push(`Project: ${normalized}`);
    return output.join("\n");
}
// ============================================================
// BUILD RELEASE
// ============================================================
export async function buildRelease(projectPath) {
    const normalized = friendlyPath(projectPath);
    if (!isFlutterProject(normalized)) {
        return `
🔴 THIS DOESN'T LOOK LIKE A FLUTTER PROJECT
...
`.trim();
    ...
    const aabPath = path.join(normalized, "build", "app", "outputs", "bundle", "release", "app-release.aab");
